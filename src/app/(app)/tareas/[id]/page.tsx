...
export default function TaskDetailPage() {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => router.back()}
        className="flex items-center gap-1.5 text-sm text-[#666] hover:text-[#111] mb-6"
      >
        <ArrowLeft size={16} />
        Volver
      </button>
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {estado === "CORRECTION_24H" && (
                  <span className="animate-pulse rounded-full bg-[#EF4444] px-2.5 py-1 text-xs font-medium text-white">
                    SLA ACTIVO
                  </span>
                )}
              </div>
              <p className="text-[#111] text-base mt-3">
                {task.descripcion || "Sin descripcion"}
              </p>
              <p className="mt-2 text-xs text-[#999]">
                Creada {timeAgo(task.createdAt)} · Actualizada{" "}
                {timeAgo(task.updatedAt)}
              </p>
            </div>
            <button
              onClick={handleDelete}
              className="rounded-lg p-2 text-[#999] hover:bg-[#FEE2E2] hover:text-[#EF4444] transition-colors"
            >
              <Trash2 size={16} />
            </button>
          </div>
          ...
          {/* Video URL */}
          {task.videoUrl && (
            <div className="bg-[#F9FAFB] rounded-lg p-3 mt-4 flex items-center gap-2">
          ...
          <div className="mt-4">
            ...
            <select
              ...
              className="text-sm border rounded-lg border-[#E5E7EB] px-3 py-1.5 outline-none"
            >
        ...
        {/* SECTION 2 - State Transition Card */}
        <div className="rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="text-sm rounded-lg border border-[#E5E7EB] px-3 py-1.5 outline-none"
            >
          ...
          {estado === "IN_EDITING" && selectedRole === "EDITOR" && (
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full text-sm rounded-lg border border-[#E5E7EB] px-3 py-2 outline-none focus:border-[#111]"
              />
            </div>
          )}
          ...
          {availableTransitions.length > 0 ? (
            ...
            <p className="text-sm text-[#999]">
              No hay transiciones disponibles para el rol seleccionado en este
              estado.
            </p>
          )}
        </div>

        {/* SECTION 3 - Corrections History */}
          ...
          <div className="rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="space-y-4">
              {task.correcciones.map((c, i) => (
                <div
                  key={c.id}
                  className="border-l-2 border-[#EF4444] pl-4 py-3"
                >
                  <div className="flex items-center gap-2 text-xs text-[#999]">
                    <span>
                      Correccion #{task.correcciones.length - i}
                    </span>
                    <span>·</span>
                  ...
                  <p className="text-sm text-[#111] mt-1">{c.detalles}</p>
      ...
      {showCorrectionModal && (
        <>
          ...
          {/* Mobile: bottom sheet */}
          <div className="lg:hidden fixed inset-x-0 bottom-0 z-50 rounded-t-2xl bg-white p-6 shadow-2xl max-h-[80vh] overflow-y-auto">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[#EF4444]">
                Solicitar Correccion (24h SLA)
              </h2>
              <button
                onClick={() => setShowCorrectionModal(false)}
            ...
            <p className="mb-4 text-sm text-[#666]">
              Al solicitar correccion, el editor tendra 24 horas habiles para
              corregir y reenviar el material.
            </p>
            <form onSubmit={handleCorrection}>
              <textarea
                ...
                className="mb-4 w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm outline-none focus:border-[#EF4444] focus:ring-1 focus:ring-[#EF4444]"
              />
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={transitioning}
                  className="flex-1 rounded-lg bg-[#EF4444] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#DC2626] disabled:opacity-50"
                >
                  {transitioning ? "Procesando..." : "Iniciar Correccion 24h"}
                </button>
              </div>
            </form>
          </div>
